...
protected:
    ...
    static int* indHeap(Root *hp,int i,bck callback); // like ind() but callback
    static void upHeap(cmpType cmpF,Root *hp,int n,bck callback);
    static void downHeap(cmpType cmpF,Root *hp,int n,bck callback);
    static int binSearch(cmpType cmpF,Root *hp,int *op,int *found);
public:
    static void setMaxCap(unsigned int maxCap){maxCapacity=maxCap;}
    static int* form(Root *hp,unsigned int const sz,int const incr);
    ...
    static int get(Root *hp,const unsigned int k);  // a=array[k]
    static void set(Root *hp,const unsigned int k,int a); // array[k]=a
    ...
    static void insert(Root *hp,const int k,int *t); 
    ...
    static int* ind(Root *hp,int i);
    static void addOrd(cmpType cmpF,Root *hp,int *op);

    // array as a stack 
    // ------------------------------------------------------------
    static void push(Root *hp,int* e);
    static int* pop(Root *hp);

    // array as a partially sorted, binary heap
    // -----------------------------------------
    static void inHeap(cmpType cmpF,Root *hp,int *e,bck callback);
    static int outHeap(cmpType cmpF,Root *hp,int *e,bck callback);
    static void updHeap(cmpType cmpF,Root *hp,int n,bck callback);
    static void delHeap(cmpType cmpF,Root *hp,int n,bck callback);

    static int* head(Root *hp); //fast way of getting array[0]
    static void reset(Root *hp,int newSz,int incr);
    static void ins(Root *hp,int k,int* t){insert(hp,k,t);}
    static void delOrd(cmpType cmpF,Root *hp,int k){ extract(hp,k); }
    static void delOrd(cmpType cmpF,Root *hp,int *obj);
    static int getBit(Root *hp,int i);
    static int setBit(Root *hp,int i,int val);
    static int getOrd(cmpType cmpF,Root *hp,int *op,int *found);
};

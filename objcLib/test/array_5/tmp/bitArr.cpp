...
// above this index. 
// --------------------------------------------------------------
void bitArr_Array::insert(Root *hp,const int k,int *t){
...
// --------------------------------------------------------------
int* bitArr_Array::form(Root *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZbitArr.array)free(hp);

    hp->ZZds.ZZbitArr.array=new int[sz];
...
void bitArr_Array::sort(cmpType cmp,Root *hp){
    ...
    qsort(hp->ZZds.ZZbitArr.array,hp->ZZds.ZZbitArr.num+1,sizeof(int),cmp);
}
void bitArr_Array::sortSubset(cmpType cmp,Root *hp,int i1,int i2){
    ...
    qsort(&(hp->ZZds.ZZbitArr.array[i1]),i2-i1+1,sizeof(int),cmp);
}
...
// --------------------------------------------------------------
int* bitArr_Array::ind(Root *hp,int i){
    if(!(hp->ZZds.ZZbitArr.array)){
...
// --------------------------------------------------------------
int* bitArr_Array::head(Root *hp){
    return (int*)(hp->ZZds.ZZbitArr.array);
}
...
// --------------------------------------------------------------
void bitArr_Array::inHeap(cmpType cmpF,Root *hp,int *e,bck callback){
    int i; int* newE;
...
// --------------------------------------------------------------
int bitArr_Array::outHeap(cmpType cmpF,Root *hp,int* e,bck callback){
    int num;
...
// --------------------------------------------------------------
int* bitArr_Array::indHeap(Root *hp,int i,bck callback){
    int k,num,sz;
    int *elem;
...
int bitArr_Array::get(Root *hp,const unsigned int k){  // a=array[k]
    int kk=k;
...
void bitArr_Array::set(Root *hp,const unsigned int k,int a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZbitArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZbitArr.num)hp->ZZds.ZZbitArr.num=kk;
...
void bitArr_Array::push(Root *hp,int* e){
    unsigned int num,sz;
...
int* bitArr_Array::pop(Root *hp){int* e=NULL; int num=hp->ZZds.ZZbitArr.num;
...
// -----------------------------------------------------------------------
void bitArr_Array::addOrd(cmpType cmpF,Root *hp,int *op){
    int k,found;
...
// elements.
// -----------------------------------------------------------------------
void bitArr_Array::delOrd(cmpType cmpF,Root *hp,int *obj){
    int k,found;
...
// -----------------------------------------------------------------------
int bitArr_Array::binSearch(cmpType cmpF,Root *hp,int *op,int *found){
...
int bitArr_Array::getBit(Root *hp,int i){
    ...
    sz=sizeof(int) * hp->ZZds.ZZbitArr.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ...
        newBytes=sizeof(int)*hp->ZZds.ZZbitArr.sz;
...
// -----------------------------------------------------------------------
int bitArr_Array::getOrd(cmpType cmpF,Root *hp,int *op,int *found){

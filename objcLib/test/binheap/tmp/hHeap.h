...
// EQUIVALENT OF:
// template <class Holder,class Element> class hHeap_Array
// --------------------------------------------------------------------
// Implementation note:
...
#ifndef ZZ_hHeap_ARRAY_INCLUDED
#define ZZ_hHeap_ARRAY_INCLUDED

// ----------------------------------------------------------
// description of the cooperating classes
// ----------------------------------------------------------
class hHeap_ArrayHolder {
public:
    unsigned int sz; // present size
    int num,incr; // higest valid index,increment
    PtrEmpl* array;
    hHeap_ArrayHolder(){ sz=0; num= -1; incr=0; array=NULL;}
};
...
protected:
    ...
    static PtrEmpl* indHeap(Header *hp,int i,bck callback); // like ind() but callback
    ...
    static int binSearch(cmpType cmpF,Header *hp,PtrEmpl *op,int *found);
public:
    ...
    static PtrEmpl get(Header *hp,const unsigned int k);  // a=array[k]
    static void set(Header *hp,const unsigned int k,PtrEmpl a); // array[k]=a
    ...
    static void insert(Header *hp,const int k,PtrEmpl *t); 
    ...
    static PtrEmpl* ind(Header *hp,int i);
    static void addOrd(cmpType cmpF,Header *hp,PtrEmpl *op);

    // array as a stack 
    // ------------------------------------------------------------
    static void push(Header *hp,PtrEmpl* e);
    static PtrEmpl* pop(Header *hp);
    ...
    static PtrEmpl* head(Header *hp); //fast way of getting array[0]
    static void reset(Header *hp,int newSz,int incr);
    static void ins(Header *hp,int k,PtrEmpl* t){insert(hp,k,t);}
    static void delOrd(cmpType cmpF,Header *hp,int k){ extract(hp,k); }
    static void delOrd(cmpType cmpF,Header *hp,PtrEmpl *obj);
    static int getBit(Header *hp,int i);
    static int setBit(Header *hp,int i,int val);
    static int getOrd(cmpType cmpF,Header *hp,PtrEmpl *op,int *found);
};

#endif // ZZ_hHeap_ARRAY_INCLUDED

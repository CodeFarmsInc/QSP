#include <stdlib.h>
// --------------------------------------------------------
unsigned int pHeap_Array::maxCapacity=0;
...
// --------------------------------------------------------------
void pHeap_Array::extract(Header *hp, const int k){
    int j,num;

    num=hp->ZZds.ZZpHeap.num;
    if(k>num)return; // nothing to do
    if(k==num){hp->ZZds.ZZpHeap.num=num-1; return;}
    for(j=k+1; j<=num; j++)hp->ZZds.ZZpHeap.array[j-1]=hp->ZZds.ZZpHeap.array[j];
    hp->ZZds.ZZpHeap.num=num-1;
}
...
// above this index. 
// --------------------------------------------------------------
void pHeap_Array::insert(Header *hp,const int k,Employee* *t){
    int j,kk,oldNum,newNum,sz;

    if(k<0)kk=0; else kk=k;
    oldNum=hp->ZZds.ZZpHeap.num;
    sz=hp->ZZds.ZZpHeap.sz;
    if(kk>oldNum) newNum=kk; else newNum=oldNum+1;
    if(newNum>=sz){
        (void)sizeChange(hp,newNum+1,0);
    }
    for(j=oldNum; j>=kk; j--) hp->ZZds.ZZpHeap.array[j+1]=hp->ZZds.ZZpHeap.array[j];
    hp->ZZds.ZZpHeap.array[kk]=(*t);
    hp->ZZds.ZZpHeap.num=newNum;
}
...
// --------------------------------------------------------------
int pHeap_Array::sizeChange(Header *hp,const unsigned int newCap,const int exact){
    Employee** a; Employee** arr;
    int nCap,j,s,sn,cap,num,incr;

    nCap=newCap;
    arr=hp->ZZds.ZZpHeap.array;
    cap=hp->ZZds.ZZpHeap.sz;
    num=hp->ZZds.ZZpHeap.num;
    incr=hp->ZZds.ZZpHeap.incr;

    if(incr==0 && !exact){
    printf("error for pHeap: attempt to change a fixed array into flexible one\n");
    return 1;
    }
    if(!arr){
        printf("error for pHeap: cannot modify capacity, array not formed\n");
        return 1;
    }

    if(nCap==cap)return 0; // nothing to do
    if(maxCapacity>0 && nCap>maxCapacity){
        printf("error for pHeap: requested capacity=%d, maxCapacity=%d\n",
                                                 nCap,maxCapacity);
        return 1;
    }
    ...
    a=new Employee*[s];
    if(!a){
        printf("error for pHeap: failed to allocate array capacity=%d\n",s);
        return 1;
    }
    ...
    delete[] arr;
    hp->ZZds.ZZpHeap.array=a;
    hp->ZZds.ZZpHeap.sz=s;
    if(num>=s)hp->ZZds.ZZpHeap.num=s-1;
...
// --------------------------------------------------------------
Employee** pHeap_Array::form(Header *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZpHeap.array)free(hp);

    hp->ZZds.ZZpHeap.array=new Employee*[sz];
    if(!hp->ZZds.ZZpHeap.array){
        printf("pHeap error, cannot allocate array size=%d\n",sz);
        return NULL;
    }
    hp->ZZds.ZZpHeap.sz=sz;
    hp->ZZds.ZZpHeap.num= -1;
    hp->ZZds.ZZpHeap.incr=incr;
    return hp->ZZds.ZZpHeap.array;
}
...
// sorted.
// --------------------------------------------------------------
void pHeap_Array::sort(cmpType cmp,Header *hp){
    if(hp->ZZds.ZZpHeap.num<=0)return; // no need to sort 0 or 1 item
    qsort(hp->ZZds.ZZpHeap.array,hp->ZZds.ZZpHeap.num+1,sizeof(Employee*),cmp);
}
void pHeap_Array::sortSubset(cmpType cmp,Header *hp,int i1,int i2){
    if(i2>hp->ZZds.ZZpHeap.num){
        printf("warning: upper range of sortSubset() automatically adjusted ");
        printf("from=%d to %d\n",i2,hp->ZZds.ZZpHeap.num);
        i2=hp->ZZds.ZZpHeap.num;
    }
    qsort(&(hp->ZZds.ZZpHeap.array[i1]),i2-i1+1,sizeof(Employee*),cmp);
}
...
// --------------------------------------------------------------
Employee** pHeap_Array::ind(Header *hp,int i){
    if(!(hp->ZZds.ZZpHeap.array)){
        printf("error for pHeap: index operation attempted before formed\n");
        return NULL;
    }
    if(i>=hp->ZZds.ZZpHeap.sz){
        if(sizeChange(hp,i+1,0))return NULL; // error message inside there
    }
    if(hp->ZZds.ZZpHeap.num<i)hp->ZZds.ZZpHeap.num=i;
    return &(hp->ZZds.ZZpHeap.array[i]);
}

// --------------------------------------------------------------
// Fast way of getting ind[0]
// --------------------------------------------------------------
Employee** pHeap_Array::head(Header *hp){
    return (Employee**)(hp->ZZds.ZZpHeap.array);
}
...
// --------------------------------------------------------------
void pHeap_Array::inHeap(cmpType cmpF,Header *hp,Employee* *e,bck callback){
    int i; Employee** newE;

    i=hp->ZZds.ZZpHeap.num + 1;
    newE=indHeap(hp,i,callback);
    if(!newE)return;
    (*newE)=(*e);
    if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[i]),i);
...
// --------------------------------------------------------------
void pHeap_Array::upHeap(cmpType cmpF,Header *hp,int n,bck callback){
    ...
    for(i1=n, i2=(n-1)/2, a1=ind(hp,n); i1>0; i1=i2, i2=(i2-1)/2, a1=a2){
        ...
        aa=(*a1); (*a1)=(*a2); (*a2)=aa;
        if(callback){
            (*callback)(&(hp->ZZds.ZZpHeap.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZpHeap.array[i2]),i2);
        }
    }
}
...
// --------------------------------------------------------------
int pHeap_Array::outHeap(cmpType cmpF,Header *hp,Employee** e,bck callback){
    int num;

    num=hp->ZZds.ZZpHeap.num;
    if(num<0) return 0;
    (*e)=hp->ZZds.ZZpHeap.array[0];
    if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[0]),-1);
    if(num>0){
       hp->ZZds.ZZpHeap.array[0]=hp->ZZds.ZZpHeap.array[num];
       if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[0]),0);
...
// --------------------------------------------------------------
void pHeap_Array::downHeap(cmpType cmpF,Header *hp,int n,bck callback){
    ...
    for(k=n, ak=ind(hp,n); k<num; k=i1, ak=a1){
        ...
        if(callback){
            (*callback)(&(hp->ZZds.ZZpHeap.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZpHeap.array[k]),k);
        }
    }
}
...
// --------------------------------------------------------------
void pHeap_Array::updHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num,n1,n2;

    num=hp->ZZds.ZZpHeap.num;
    if(n<0 || n>num){
        printf("error updating heap=pHeap, n=%d num=%d\n",n,num);
        return;
    }
...
// --------------------------------------------------------------
void pHeap_Array::delHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num; Employee** arr;

    num=hp->ZZds.ZZpHeap.num;
    arr=hp->ZZds.ZZpHeap.array;
    if(n<0 || n>num){
        printf("error deleting from heap=pHeap, n=%d num=%d\n",n,num);
        return;
    }
    (hp->ZZds.ZZpHeap.num)--;
    if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[num]),-1);
    if(n==num) return;
    arr[n]=arr[num];
    if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[n]),n);
...
// --------------------------------------------------------------
Employee** pHeap_Array::indHeap(Header *hp,int i,bck callback){
    int k,num,sz;
    Employee* *elem;
    ...
    if(i>=sz){
        for(k=0; k<num; k++){
            if(callback) (*callback)(&(hp->ZZds.ZZpHeap.array[k]),k);
        }
    }
    return elem;
}
...
int pHeap_Array::formed(Header *hp){ if(hp->ZZds.ZZpHeap.array)return 1; return 0;}

void pHeap_Array::free(Header *hp){ 
    if(hp->ZZds.ZZpHeap.array){
        delete[] hp->ZZds.ZZpHeap.array;
        hp->ZZds.ZZpHeap.array=NULL;
    }
    hp->ZZds.ZZpHeap.num= -1;
    hp->ZZds.ZZpHeap.sz=0;
}

unsigned int pHeap_Array::capacity(Header *hp) {return hp->ZZds.ZZpHeap.sz;} 

unsigned int pHeap_Array::size(Header *hp) {return (unsigned int)(hp->ZZds.ZZpHeap.num + 1);} 

int pHeap_Array::increment(Header *hp) {return hp->ZZds.ZZpHeap.incr;} 

Employee* pHeap_Array::get(Header *hp,const unsigned int k){  // a=array[k]
    int kk=k;
    if(kk>=hp->ZZds.ZZpHeap.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZpHeap.num)hp->ZZds.ZZpHeap.num=kk;
    return hp->ZZds.ZZpHeap.array[kk];
}

void pHeap_Array::set(Header *hp,const unsigned int k,Employee* a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZpHeap.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZpHeap.num)hp->ZZds.ZZpHeap.num=kk;
    hp->ZZds.ZZpHeap.array[kk]=a;
}
void pHeap_Array::remove(Header *hp,const unsigned int k){ // fast but order changed
    int kk=k;
    int num=hp->ZZds.ZZpHeap.num;
    if(kk<num){hp->ZZds.ZZpHeap.array[k]=hp->ZZds.ZZpHeap.array[num]; (hp->ZZds.ZZpHeap.num)--;}
}
int pHeap_Array::reduce(Header *hp){             // reduce the array to its used size
    return sizeChange(hp,hp->ZZds.ZZpHeap.num+1,1);
}
int pHeap_Array::reduce(Header *hp,const unsigned int newCap){ // reduce size to newSz
    if(newCap>=hp->ZZds.ZZpHeap.sz)return hp->ZZds.ZZpHeap.sz;
    return sizeChange(hp,newCap,1);
} 
int pHeap_Array::grow(Header *hp,const unsigned int newCap){
    return sizeChange(hp,newCap,0);
} 
void pHeap_Array::push(Header *hp,Employee** e){
    unsigned int num,sz;

    num=hp->ZZds.ZZpHeap.num+1; sz=hp->ZZds.ZZpHeap.sz;
    if(num>=sz)sizeChange(hp,num+1,0);
    hp->ZZds.ZZpHeap.array[num]=(*e); (hp->ZZds.ZZpHeap.num)++;
}
Employee** pHeap_Array::pop(Header *hp){Employee** e=NULL; int num=hp->ZZds.ZZpHeap.num;
    if(num>=0){e= &(hp->ZZds.ZZpHeap.array[num]); (hp->ZZds.ZZpHeap.num)--;}
    return e;
}
void pHeap_Array::reset(Header *hp,int newSz,int incr){
    hp->ZZds.ZZpHeap.num=newSz-1; hp->ZZds.ZZpHeap.incr=incr;
}
...
// -----------------------------------------------------------------------
void pHeap_Array::addOrd(cmpType cmpF,Header *hp,Employee* *op){
    int k,found;

    if(hp->ZZds.ZZpHeap.num<0)k=0;
...
// elements.
// -----------------------------------------------------------------------
void pHeap_Array::delOrd(cmpType cmpF,Header *hp,Employee* *obj){
    int k,found;

    k=binSearch(cmpF,hp,obj,&found);
    if(found) delOrd(cmpF,hp,k);
    else printf("warning: calling pHeap::delOrd() but object not there\n");
}
...
// -----------------------------------------------------------------------
int pHeap_Array::binSearch(cmpType cmpF,Header *hp,Employee* *op,int *found){
    int i1,i2,k,cmp,num; Employee** rr; Employee** arr;

    arr=hp->ZZds.ZZpHeap.array;
    num=hp->ZZds.ZZpHeap.num;
    if(!arr || num<0){
        printf("pHeap error in binSearch(): array not formed or empty\n");
        return  -1;
    }
    i1=0; i2=num; k= -1;
...
// -----------------------------------------------------------------------
int pHeap_Array::getBit(Header *hp,int i){
    int iChar,iBit,sz,k,newBytes; Employee** ap; unsigned char *p,b;

    sz=sizeof(Employee*) * hp->ZZds.ZZpHeap.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ap=ind(hp,i/(8*sizeof(Employee*)));
        if(!ap){
            printf("pHeap error: cannot realocate for %d bits\n",i);
            return -1;
        }

        // initialize the new part of the array to 0
        newBytes=sizeof(Employee*)*hp->ZZds.ZZpHeap.sz;
        p=(unsigned char*)(hp->ZZds.ZZpHeap.array);
        for(k=sz; k<newBytes; k++) p[k]=0;
    }
    else if(hp->ZZds.ZZpHeap.num<0){ // first call, initialize all bits to 0
        p=(unsigned char*)(hp->ZZds.ZZpHeap.array);
        for(k=0; k<sz; k++) p[k]=0;
    }

    k=i/(8*sizeof(Employee*)); // object index in which bit i is
    if(hp->ZZds.ZZpHeap.num < k)hp->ZZds.ZZpHeap.num=k;
...
// -----------------------------------------------------------------------
int pHeap_Array::setBit(Header *hp,int i,int val){
    int iChar,iBit,sz,k,newBytes; Employee** ap; unsigned char *p,b;
...
// -----------------------------------------------------------------------
int pHeap_Array::getOrd(cmpType cmpF,Header *hp,Employee* *op,int *found){

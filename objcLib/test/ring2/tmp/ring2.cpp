...
int ring2_Ring2::debugFun(A *tail){
    A *p,*nxt,*head; int h,t,ret;
    ring2_Ring2Iterator it;

    ret=0;
    for(p=it.fromHead(tail), h=0, head=NULL; p; p=it.next(), h++){
        if(h==0)head=p;
        nxt=p->ZZds.ZZring2.next;
        if(nxt->ZZds.ZZring2.prev!=p ||nxt==NULL){
            printf("debug error fromHead index=%d\n",h);
            ret=1;
            break;
        }
    }
    if(!head || head->ZZds.ZZring2.prev!=tail || tail->ZZds.ZZring2.next!=head){
        printf("debug tail-head error fromHead\n");
        ret=1;
    }

    for(p=it.fromTail(tail), t=0, head=NULL; p; p=it.next(), t++){
        nxt=p->ZZds.ZZring2.prev;
        if(nxt==tail)head=p;
        if(nxt->ZZds.ZZring2.next!=p ||nxt==NULL){
...
A* const ring2_Ring2::next(A *tail, A *c){
    A* ret=c->ZZds.ZZring2.next;
    if(c==tail)ret=NULL;
    return ret; // return NULL when c is the tail
}

A* const ring2_Ring2::prev(A *tail, A *c){
    A* ret=c->ZZds.ZZring2.prev;
    if(ret==tail)ret=NULL;
    return ret; // return NULL when c is the tail
}

A* const ring2_Ring2::nextRing(A *c){
    return c->ZZds.ZZring2.next;
}

A* const ring2_Ring2::prevRing(A *c){
    return c->ZZds.ZZring2.prev;
}

A* ring2_Ring2::addHead(A *tail, A *c){
    A *head;

    if(c->ZZds.ZZring2.next || c->ZZds.ZZring2.prev){
        printf("ring2.addHead() error: element=%d already in ring2\n",c);
        return NULL;
    }
    if(tail){
        head=tail->ZZds.ZZring2.next;
        c->ZZds.ZZring2.next=head; tail->ZZds.ZZring2.next=c;
        c->ZZds.ZZring2.prev=tail; head->ZZds.ZZring2.prev=c;
    }
    else {tail=c; c->ZZds.ZZring2.next=c; c->ZZds.ZZring2.prev=c;}
    return tail; 
}
                          
A* ring2_Ring2::addTail(A *tail, A *c){
...
A* ring2_Ring2::append(A *tail,A *c1, A *c2){
    A *nxt;
    ...
    nxt=c1->ZZds.ZZring2.next;
    c2->ZZds.ZZring2.next=nxt; c1->ZZds.ZZring2.next=c2;
    nxt->ZZds.ZZring2.prev=c2; c2->ZZds.ZZring2.prev=c1;
    if(tail==c1)tail=c2;
    return tail;
}
...
void ring2_Ring2::insert(A *c1, A *c2){
    A *prv;

    if(c1->ZZds.ZZring2.next || c1->ZZds.ZZring2.prev){
        printf("ring2.insert() error: element=%d already in ring2\n",c1);
        return;
    }
    if(c2->ZZds.ZZring2.next==NULL || c2->ZZds.ZZring2.prev==NULL){
        printf("ring2.insert() error: element=%d not in ring2\n",c2);
        return;
    }
    prv=c2->ZZds.ZZring2.prev;
    prv->ZZds.ZZring2.next=c1; c1->ZZds.ZZring2.next=c2;
    c2->ZZds.ZZring2.prev=c1; c1->ZZds.ZZring2.prev=prv;
}
...
A* ring2_Ring2::remove(A *tail, A *c){
    A *prv,*nxt; A *t;

    t=tail;
    if(c->ZZds.ZZring2.next==NULL || c->ZZds.ZZring2.prev==NULL){
        printf("ring2:remove() error: node=%d not on the list\n",c); return NULL;
    }
    nxt=c->ZZds.ZZring2.next;
    prv=c->ZZds.ZZring2.prev;
    if(c==nxt)t=NULL;
    else {
        if(c==t)t=prv;
        prv->ZZds.ZZring2.next=nxt;
        nxt->ZZds.ZZring2.prev=prv;
    }
    c->ZZds.ZZring2.next=c->ZZds.ZZring2.prev=NULL;
    return t;
}
...
A* ring2_Ring2::sort(ZZsortFun cmp, A *tail){
    ...
    for(p=subs=last=tail->ZZds.ZZring2.next, closeSublist=0; p; p=nxt){
        if(closeSublist){
            last->ZZds.ZZring2.prev=p;
            last=p;
            closeSublist=0;
        }
        ...
        if(p==tail){
            ...
            nxt=p->ZZds.ZZring2.next;
            if((*cmp)(p,p->ZZds.ZZring2.next)>0)closeSublist=1;
        }

        if(closeSublist)p->ZZds.ZZring2.next=NULL;
    }
    last->ZZds.ZZring2.prev=NULL; // close the chain
    ...
    for(stopFlg=0; !stopFlg;){ // keep repeating
        ...
        for(a1=subs, lastA1=NULL; a1; a1=nxt){
            a2=a1->ZZds.ZZring2.prev;
            if(a2==NULL){
                if(subs==a1)stopFlg=1;
                else { lastA1->ZZds.ZZring2.prev=a1; a1->ZZds.ZZring2.prev=NULL;}
                break; // last odd sublist, do nothing
            }
            nxt=a2->ZZds.ZZring2.prev;
            ...
            for(t1=a1, t2=a2, last=NULL; t1||t2; ){
                ...
                if(choice==1){ t=t1; t1=t1->ZZds.ZZring2.next; }
                else         { t=t2; t2=t2->ZZds.ZZring2.next; }

                if(last==NULL){
                    if(lastA1==NULL)subs=t;
                    else lastA1->ZZds.ZZring2.prev=t;
                    lastA1=t; lastA1->ZZds.ZZring2.prev=NULL;
                }
                else last->ZZds.ZZring2.next=t;
    ...
    for(p=subs; p; p=nxt){
        nxt=p->ZZds.ZZring2.next;
        if(nxt)nxt->ZZds.ZZring2.prev=p;
        else {
            subs->ZZds.ZZring2.prev=p;
            p->ZZds.ZZring2.next=subs;
        }
    }
    return tail;
}
...
void ring2_Ring2::merge(A *s,A* t){
    A *u,*v;
    ...
    t->ZZds.ZZring2.next=u; u->ZZds.ZZring2.prev=t;
    s->ZZds.ZZring2.next=v; v->ZZds.ZZring2.prev=s;
}


A* ring2_Ring2Iterator::fromHead(A *p){ 
    A *ret;
...
A* ring2_Ring2Iterator::fromTail(A *p){ 
    dir=1;
    if(p==NULL){nxt=tail=NULL; return NULL;}
    tail=p;
    nxt=tail->ZZds.ZZring2.prev;
    if(nxt==tail)nxt=tail=NULL; 
    return p;
}


A* const ring2_Ring2Iterator::next(){ 
    A *c;

    c=nxt;
    if(dir==0){ if(c==tail)nxt=tail=NULL; else nxt=c->ZZds.ZZring2.next; }
    else      { if(c==tail)c=nxt=tail=NULL; else nxt=c->ZZds.ZZring2.prev; }
    return(c);
}


void ring2_Ring2Iterator::start(A *p){ 
    A *ret;
...
A* const ring2_Ring2Iterator::operator++(){ 
    A *ret;

    if(nxt){
        ret=nxt; 
        if(ret==tail)tail=nxt=NULL;
        else nxt=nxt->ZZds.ZZring2.next;
    }
    else if(tail) {
        ret=tail->ZZds.ZZring2.next;
...
A* const ring2_Ring2Iterator::operator--(){ 
    ...
    if(nxt){
        ...
        else nxt=nxt->ZZds.ZZring2.prev;
    }
    else if(tail) {
        ret=tail;
        if(ret==tail->ZZds.ZZring2.next)tail=NULL;
        else nxt=ret->ZZds.ZZring2.prev;
    }
    else ret=NULL;
    return ret;
}

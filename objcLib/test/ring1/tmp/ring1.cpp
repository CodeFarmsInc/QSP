...
A* const ring1_Ring1::next(A *tail, A *c){
    A* ret=c->ZZds.ZZring1.next;
    if(c==tail)ret=NULL;
    return ret; // return NULL when c is the externally given tail
}

A* const ring1_Ring1::nextRing(A *c){
    return c->ZZds.ZZring1.next;
}

A* ring1_Ring1::addHead(A *tail, A *c){
    if(c->ZZds.ZZring1.next){
        printf("ring1.addHead() error: element=%d already in ring1\n",c);
        return NULL;
    }
    if(tail){c->ZZds.ZZring1.next=tail->ZZds.ZZring1.next; tail->ZZds.ZZring1.next=c;}
    else        {tail=c; c->ZZds.ZZring1.next=c;}
    return tail; 
}
                          
A* ring1_Ring1::addTail(A *tail, A *c){
    if(c->ZZds.ZZring1.next){
...
A* ring1_Ring1::append(A *tail,A *c1, A *c2){
    A *x;
    ...
    c2->ZZds.ZZring1.next=c1->ZZds.ZZring1.next;
    c1->ZZds.ZZring1.next=c2;
    if(tail==c1)tail=c2;
    return tail;
}
...
A* ring1_Ring1::remove(A *tail, A *c){
    A *x,*nxt;

    for(x=tail, nxt=NULL; x!=tail || nxt==NULL; x=nxt){
        nxt=x->ZZds.ZZring1.next;
        if(nxt==c)break;
    }
    if(!x || x->ZZds.ZZring1.next!=c){
        printf("ring1:remove() error: node not on the list\n"); return NULL;
    }
    if(c==x)tail=NULL;
    else if(c==tail)tail=x;
    x->ZZds.ZZring1.next=c->ZZds.ZZring1.next;
    c->ZZds.ZZring1.next=NULL;
    return tail;
}
...
A* ring1_Ring1::sort(ZZsortFun cmp, A *tail){
    ...
    if(!temp){
        printf("ring1 Ring1::sort cannot allocate temporary node\n"); return NULL;
    }
    temp->ZZds.ZZring1.next=tail->ZZds.ZZring1.next;
    tail->ZZds.ZZring1.next=temp;

    // keep sorting adjacent sublists until everything is one list
    for(stopFlg=0; !stopFlg;){ // keep repeating

        // process all sublist pairs
        for(a1=temp->ZZds.ZZring1.next, last=temp; a1!=temp; a1=nxt){

            // detect the first sublist
            for(b1=a1; b1->ZZds.ZZring1.next!=temp; b1=b1->ZZds.ZZring1.next){
                if((*cmp)(b1,b1->ZZds.ZZring1.next)>0)break;
            }
   
            if(b1->ZZds.ZZring1.next==temp){ // last of the odd number of sublists
                if(a1==temp->ZZds.ZZring1.next) stopFlg=1; // list fully sorted
                last->ZZds.ZZring1.next=a1;
                last=b1;
                break;
            }

            // detect the second sublist
            a2=b1->ZZds.ZZring1.next;
            for(b2=a2; b2->ZZds.ZZring1.next!=temp; b2=b2->ZZds.ZZring1.next){
                if((*cmp)(b2,b2->ZZds.ZZring1.next)>0)break;
            }
            nxt=b2->ZZds.ZZring1.next; // remember it before b2 changes its position
            ...
            for(t1=a1, t2=a2; t1||t2; ){
                ...
                if(choice==1){
                    last->ZZds.ZZring1.next=t1;
                    last=t1;
                    if(t1==b1)t1=NULL; else t1=t1->ZZds.ZZring1.next;
                }
                else {
                    last->ZZds.ZZring1.next=t2;
                    last=t2;
                    if(t2==b2)t2=NULL; else t2=t2->ZZds.ZZring1.next;
                }
            }
        }
        last->ZZds.ZZring1.next=temp;
        tail=last;
    }
    
    // remove the temporary node
    tail->ZZds.ZZring1.next=temp->ZZds.ZZring1.next;
    delete temp;
    return tail;
}
...
void ring1_Ring1::merge(A *s,A* t){
    A *u,*v;
    ...
    t->ZZds.ZZring1.next=u; 
    s->ZZds.ZZring1.next=v;
}


A* ring1_Ring1Iterator::fromHead(A *p){ 
    A *ret;
...
A* const ring1_Ring1Iterator::next(){ 
    A *c;

    c=nxt;
    if(c==tail)nxt=tail=NULL; else nxt=c->ZZds.ZZring1.next;
    return(c);
}


void ring1_Ring1Iterator::start(A *p){ 
    A *ret;
...
A* const ring1_Ring1Iterator::operator++(){ 
    A *ret;

    if(tail){
        if(nxt){
            ret=nxt; 
            nxt=nxt->ZZds.ZZring1.next;

...
main(int argc,char **argv) {
    ...
    FILE *file1,*file2;
    Header *h;
    Employee *ep,*sp,**pa,*oa,*p1,**p2,*p3,*p4,*ee,**pp,**pe;
    ...
    /* load hArr with the same data as before */
    eIter.start(sp);
    ITERATE(eIter,ep){
        hArr.addTail(h,ep);
        fprintf(file2,"in heap: %s emplNum=%d salary=%d\n",
    ...
    for(i=0; i<hArr.size(h); i++){
        ep=hArr.get(h,i);
        fprintf(file2,"sorted: %s emplNum=%d salary=%d\n",
                                                 cp,ep->emplNum,ep->salary);
    }
    return(0);
}

...
// above this index. 
// --------------------------------------------------------------
void hArr_Array::insert(Header *hp,const int k,void* *t){
...
// --------------------------------------------------------------
void** hArr_Array::form(Header *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZhArr.array)free(hp);

    hp->ZZds.ZZhArr.array=new void*[sz];
...
void hArr_Array::sort(cmpType cmp,Header *hp){
    ...
    qsort(hp->ZZds.ZZhArr.array,hp->ZZds.ZZhArr.num+1,sizeof(void*),cmp);
}
void hArr_Array::sortSubset(cmpType cmp,Header *hp,int i1,int i2){
    ...
    qsort(&(hp->ZZds.ZZhArr.array[i1]),i2-i1+1,sizeof(void*),cmp);
}
...
// --------------------------------------------------------------
void** hArr_Array::ind(Header *hp,int i){
    if(!(hp->ZZds.ZZhArr.array)){
...
// --------------------------------------------------------------
void** hArr_Array::head(Header *hp){
    return (void**)(hp->ZZds.ZZhArr.array);
}
...
// --------------------------------------------------------------
void hArr_Array::inHeap(cmpType cmpF,Header *hp,void* *e,bck callback){
    int i; void** newE;
...
// --------------------------------------------------------------
int hArr_Array::outHeap(cmpType cmpF,Header *hp,void** e,bck callback){
    int num;
...
// --------------------------------------------------------------
void** hArr_Array::indHeap(Header *hp,int i,bck callback){
    int k,num,sz;
    void* *elem;
...
void* hArr_Array::get(Header *hp,const unsigned int k){  // a=array[k]
    int kk=k;
...
void hArr_Array::set(Header *hp,const unsigned int k,void* a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZhArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZhArr.num)hp->ZZds.ZZhArr.num=kk;
...
void hArr_Array::push(Header *hp,void** e){
    unsigned int num,sz;
...
void** hArr_Array::pop(Header *hp){void** e=NULL; int num=hp->ZZds.ZZhArr.num;
...
// -----------------------------------------------------------------------
void hArr_Array::addOrd(cmpType cmpF,Header *hp,void* *op){
    int k,found;
...
// elements.
// -----------------------------------------------------------------------
void hArr_Array::delOrd(cmpType cmpF,Header *hp,void* *obj){
    int k,found;
...
// -----------------------------------------------------------------------
int hArr_Array::binSearch(cmpType cmpF,Header *hp,void* *op,int *found){
...
int hArr_Array::getBit(Header *hp,int i){
    ...
    sz=sizeof(void*) * hp->ZZds.ZZhArr.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ...
        newBytes=sizeof(void*)*hp->ZZds.ZZhArr.sz;
...
// -----------------------------------------------------------------------
int hArr_Array::getOrd(cmpType cmpF,Header *hp,void* *op,int *found){

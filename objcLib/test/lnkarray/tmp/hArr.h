...
#ifndef ZZ_hArr_ARRAY_INCLUDED
#define ZZ_hArr_ARRAY_INCLUDED

// ----------------------------------------------------------
// description of the cooperating classes
// ----------------------------------------------------------
class hArr_ArrayHolder {
public:
    unsigned int sz; // present size
    int num,incr; // higest valid index,increment
    void** array;
    hArr_ArrayHolder(){ sz=0; num= -1; incr=0; array=NULL;}
};
...
protected:
    ...
    static void** indHeap(Header *hp,int i,bck callback); // like ind() but callback
    ...
    static int binSearch(cmpType cmpF,Header *hp,void* *op,int *found);
public:
    static void setMaxCap(unsigned int maxCap){maxCapacity=maxCap;}
    static void** form(Header *hp,unsigned int const sz,int const incr);
    ...
    static void* get(Header *hp,const unsigned int k);  // a=array[k]
    static void set(Header *hp,const unsigned int k,void* a); // array[k]=a
    ...
    static void insert(Header *hp,const int k,void* *t); 
    ...
    static void** ind(Header *hp,int i);
    static void addOrd(cmpType cmpF,Header *hp,void* *op);

    // array as a stack 
    // ------------------------------------------------------------
    static void push(Header *hp,void** e);
    static void** pop(Header *hp);

    // array as a partially sorted, binary heap
    // -----------------------------------------
    static void inHeap(cmpType cmpF,Header *hp,void* *e,bck callback);
    static int outHeap(cmpType cmpF,Header *hp,void* *e,bck callback);
    ...
    static void** head(Header *hp); //fast way of getting array[0]
    static void reset(Header *hp,int newSz,int incr);
    static void ins(Header *hp,int k,void** t){insert(hp,k,t);}
    static void delOrd(cmpType cmpF,Header *hp,int k){ extract(hp,k); }
    static void delOrd(cmpType cmpF,Header *hp,void* *obj);
    static int getBit(Header *hp,int i);
    static int setBit(Header *hp,int i,int val);
    static int getOrd(cmpType cmpF,Header *hp,void* *op,int *found);
};

#endif // ZZ_hArr_ARRAY_INCLUDED

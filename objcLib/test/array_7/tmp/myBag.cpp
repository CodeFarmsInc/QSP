...
// above this index. 
// --------------------------------------------------------------
void myBag_Array::insert(Root *hp,const int k,void* *t){
...
// --------------------------------------------------------------
void** myBag_Array::form(Root *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZmyBag.array)free(hp);

    hp->ZZds.ZZmyBag.array=new void*[sz];
...
void myBag_Array::sort(cmpType cmp,Root *hp){
    ...
    qsort(hp->ZZds.ZZmyBag.array,hp->ZZds.ZZmyBag.num+1,sizeof(void*),cmp);
}
void myBag_Array::sortSubset(cmpType cmp,Root *hp,int i1,int i2){
    ...
    qsort(&(hp->ZZds.ZZmyBag.array[i1]),i2-i1+1,sizeof(void*),cmp);
}
...
// --------------------------------------------------------------
void** myBag_Array::ind(Root *hp,int i){
    if(!(hp->ZZds.ZZmyBag.array)){
...
// --------------------------------------------------------------
void** myBag_Array::head(Root *hp){
    return (void**)(hp->ZZds.ZZmyBag.array);
}
...
// --------------------------------------------------------------
void myBag_Array::inHeap(cmpType cmpF,Root *hp,void* *e,bck callback){
    int i; void** newE;
...
// --------------------------------------------------------------
int myBag_Array::outHeap(cmpType cmpF,Root *hp,void** e,bck callback){
    int num;
...
// --------------------------------------------------------------
void** myBag_Array::indHeap(Root *hp,int i,bck callback){
    int k,num,sz;
    void* *elem;
...
void* myBag_Array::get(Root *hp,const unsigned int k){  // a=array[k]
    int kk=k;
...
void myBag_Array::set(Root *hp,const unsigned int k,void* a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZmyBag.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZmyBag.num)hp->ZZds.ZZmyBag.num=kk;
...
void myBag_Array::push(Root *hp,void** e){
    unsigned int num,sz;
...
void** myBag_Array::pop(Root *hp){void** e=NULL; int num=hp->ZZds.ZZmyBag.num;
...
// -----------------------------------------------------------------------
void myBag_Array::addOrd(cmpType cmpF,Root *hp,void* *op){
    int k,found;
...
// elements.
// -----------------------------------------------------------------------
void myBag_Array::delOrd(cmpType cmpF,Root *hp,void* *obj){
    int k,found;
...
// -----------------------------------------------------------------------
int myBag_Array::binSearch(cmpType cmpF,Root *hp,void* *op,int *found){
...
int myBag_Array::getBit(Root *hp,int i){
    ...
    sz=sizeof(void*) * hp->ZZds.ZZmyBag.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ...
        newBytes=sizeof(void*)*hp->ZZds.ZZmyBag.sz;
...
// -----------------------------------------------------------------------
int myBag_Array::getOrd(cmpType cmpF,Root *hp,void* *op,int *found){

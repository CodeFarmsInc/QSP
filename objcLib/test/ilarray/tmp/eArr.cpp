#include <stdlib.h>
// --------------------------------------------------------
unsigned int eArr_Array::maxCapacity=0;
...
// --------------------------------------------------------------
void eArr_Array::extract(Department *hp, const int k){
    int j,num;

    num=hp->ZZds.ZZeArr.num;
    if(k>num)return; // nothing to do
    if(k==num){hp->ZZds.ZZeArr.num=num-1; return;}
    for(j=k+1; j<=num; j++)hp->ZZds.ZZeArr.array[j-1]=hp->ZZds.ZZeArr.array[j];
    hp->ZZds.ZZeArr.num=num-1;
}
...
// above this index. 
// --------------------------------------------------------------
void eArr_Array::insert(Department *hp,const int k,void* *t){
    int j,kk,oldNum,newNum,sz;

    if(k<0)kk=0; else kk=k;
    oldNum=hp->ZZds.ZZeArr.num;
    sz=hp->ZZds.ZZeArr.sz;
    if(kk>oldNum) newNum=kk; else newNum=oldNum+1;
    if(newNum>=sz){
        (void)sizeChange(hp,newNum+1,0);
    }
    for(j=oldNum; j>=kk; j--) hp->ZZds.ZZeArr.array[j+1]=hp->ZZds.ZZeArr.array[j];
    hp->ZZds.ZZeArr.array[kk]=(*t);
    hp->ZZds.ZZeArr.num=newNum;
}
...
// --------------------------------------------------------------
int eArr_Array::sizeChange(Department *hp,const unsigned int newCap,const int exact){
    void** a; void** arr;
    int nCap,j,s,sn,cap,num,incr;

    nCap=newCap;
    arr=hp->ZZds.ZZeArr.array;
    cap=hp->ZZds.ZZeArr.sz;
    num=hp->ZZds.ZZeArr.num;
    incr=hp->ZZds.ZZeArr.incr;

    if(incr==0 && !exact){
    printf("error for eArr: attempt to change a fixed array into flexible one\n");
    return 1;
    }
    if(!arr){
        printf("error for eArr: cannot modify capacity, array not formed\n");
        return 1;
    }

    if(nCap==cap)return 0; // nothing to do
    if(maxCapacity>0 && nCap>maxCapacity){
        printf("error for eArr: requested capacity=%d, maxCapacity=%d\n",
                                                 nCap,maxCapacity);
        return 1;
    }
    ...
    a=new void*[s];
    if(!a){
        printf("error for eArr: failed to allocate array capacity=%d\n",s);
        return 1;
    }
    ...
    delete[] arr;
    hp->ZZds.ZZeArr.array=a;
    hp->ZZds.ZZeArr.sz=s;
    if(num>=s)hp->ZZds.ZZeArr.num=s-1;
...
// --------------------------------------------------------------
void** eArr_Array::form(Department *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZeArr.array)free(hp);

    hp->ZZds.ZZeArr.array=new void*[sz];
    if(!hp->ZZds.ZZeArr.array){
        printf("eArr error, cannot allocate array size=%d\n",sz);
        return NULL;
    }
    hp->ZZds.ZZeArr.sz=sz;
    hp->ZZds.ZZeArr.num= -1;
    hp->ZZds.ZZeArr.incr=incr;
    return hp->ZZds.ZZeArr.array;
}
...
// sorted.
// --------------------------------------------------------------
void eArr_Array::sort(cmpType cmp,Department *hp){
    if(hp->ZZds.ZZeArr.num<=0)return; // no need to sort 0 or 1 item
    qsort(hp->ZZds.ZZeArr.array,hp->ZZds.ZZeArr.num+1,sizeof(void*),cmp);
}
void eArr_Array::sortSubset(cmpType cmp,Department *hp,int i1,int i2){
    if(i2>hp->ZZds.ZZeArr.num){
        printf("warning: upper range of sortSubset() automatically adjusted ");
        printf("from=%d to %d\n",i2,hp->ZZds.ZZeArr.num);
        i2=hp->ZZds.ZZeArr.num;
    }
    qsort(&(hp->ZZds.ZZeArr.array[i1]),i2-i1+1,sizeof(void*),cmp);
}
...
// --------------------------------------------------------------
void** eArr_Array::ind(Department *hp,int i){
    if(!(hp->ZZds.ZZeArr.array)){
        printf("error for eArr: index operation attempted before formed\n");
        return NULL;
    }
    if(i>=hp->ZZds.ZZeArr.sz){
        if(sizeChange(hp,i+1,0))return NULL; // error message inside there
    }
    if(hp->ZZds.ZZeArr.num<i)hp->ZZds.ZZeArr.num=i;
    return &(hp->ZZds.ZZeArr.array[i]);
}

// --------------------------------------------------------------
// Fast way of getting ind[0]
// --------------------------------------------------------------
void** eArr_Array::head(Department *hp){
    return (void**)(hp->ZZds.ZZeArr.array);
}
...
// --------------------------------------------------------------
void eArr_Array::inHeap(cmpType cmpF,Department *hp,void* *e,bck callback){
    int i; void** newE;

    i=hp->ZZds.ZZeArr.num + 1;
    newE=indHeap(hp,i,callback);
    if(!newE)return;
    (*newE)=(*e);
    if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[i]),i);
...
// --------------------------------------------------------------
void eArr_Array::upHeap(cmpType cmpF,Department *hp,int n,bck callback){
    ...
    for(i1=n, i2=(n-1)/2, a1=ind(hp,n); i1>0; i1=i2, i2=(i2-1)/2, a1=a2){
        ...
        aa=(*a1); (*a1)=(*a2); (*a2)=aa;
        if(callback){
            (*callback)(&(hp->ZZds.ZZeArr.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZeArr.array[i2]),i2);
        }
    }
}
...
// --------------------------------------------------------------
int eArr_Array::outHeap(cmpType cmpF,Department *hp,void** e,bck callback){
    int num;

    num=hp->ZZds.ZZeArr.num;
    if(num<0) return 0;
    (*e)=hp->ZZds.ZZeArr.array[0];
    if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[0]),-1);
    if(num>0){
       hp->ZZds.ZZeArr.array[0]=hp->ZZds.ZZeArr.array[num];
       if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[0]),0);
...
// --------------------------------------------------------------
void eArr_Array::downHeap(cmpType cmpF,Department *hp,int n,bck callback){
    ...
    for(k=n, ak=ind(hp,n); k<num; k=i1, ak=a1){
        ...
        if(callback){
            (*callback)(&(hp->ZZds.ZZeArr.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZeArr.array[k]),k);
        }
    }
}
...
// --------------------------------------------------------------
void eArr_Array::updHeap(cmpType cmpF,Department *hp,int n,bck callback){
    int num,n1,n2;

    num=hp->ZZds.ZZeArr.num;
    if(n<0 || n>num){
        printf("error updating heap=eArr, n=%d num=%d\n",n,num);
        return;
    }
...
// --------------------------------------------------------------
void eArr_Array::delHeap(cmpType cmpF,Department *hp,int n,bck callback){
    int num; void** arr;

    num=hp->ZZds.ZZeArr.num;
    arr=hp->ZZds.ZZeArr.array;
    if(n<0 || n>num){
        printf("error deleting from heap=eArr, n=%d num=%d\n",n,num);
        return;
    }
    (hp->ZZds.ZZeArr.num)--;
    if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[num]),-1);
    if(n==num) return;
    arr[n]=arr[num];
    if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[n]),n);
...
// --------------------------------------------------------------
void** eArr_Array::indHeap(Department *hp,int i,bck callback){
    int k,num,sz;
    void* *elem;
    ...
    if(i>=sz){
        for(k=0; k<num; k++){
            if(callback) (*callback)(&(hp->ZZds.ZZeArr.array[k]),k);
        }
    }
    return elem;
}
...
int eArr_Array::formed(Department *hp){ if(hp->ZZds.ZZeArr.array)return 1; return 0;}

void eArr_Array::free(Department *hp){ 
    if(hp->ZZds.ZZeArr.array){
        delete[] hp->ZZds.ZZeArr.array;
        hp->ZZds.ZZeArr.array=NULL;
    }
    hp->ZZds.ZZeArr.num= -1;
    hp->ZZds.ZZeArr.sz=0;
}

unsigned int eArr_Array::capacity(Department *hp) {return hp->ZZds.ZZeArr.sz;} 

unsigned int eArr_Array::size(Department *hp) {return (unsigned int)(hp->ZZds.ZZeArr.num + 1);} 

int eArr_Array::increment(Department *hp) {return hp->ZZds.ZZeArr.incr;} 

void* eArr_Array::get(Department *hp,const unsigned int k){  // a=array[k]
    int kk=k;
    if(kk>=hp->ZZds.ZZeArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZeArr.num)hp->ZZds.ZZeArr.num=kk;
    return hp->ZZds.ZZeArr.array[kk];
}

void eArr_Array::set(Department *hp,const unsigned int k,void* a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZeArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZeArr.num)hp->ZZds.ZZeArr.num=kk;
    hp->ZZds.ZZeArr.array[kk]=a;
}
void eArr_Array::remove(Department *hp,const unsigned int k){ // fast but order changed
    int kk=k;
    int num=hp->ZZds.ZZeArr.num;
    if(kk<num){hp->ZZds.ZZeArr.array[k]=hp->ZZds.ZZeArr.array[num]; (hp->ZZds.ZZeArr.num)--;}
}
int eArr_Array::reduce(Department *hp){             // reduce the array to its used size
    return sizeChange(hp,hp->ZZds.ZZeArr.num+1,1);
}
int eArr_Array::reduce(Department *hp,const unsigned int newCap){ // reduce size to newSz
    if(newCap>=hp->ZZds.ZZeArr.sz)return hp->ZZds.ZZeArr.sz;
    return sizeChange(hp,newCap,1);
} 
int eArr_Array::grow(Department *hp,const unsigned int newCap){
    return sizeChange(hp,newCap,0);
} 
void eArr_Array::push(Department *hp,void** e){
    unsigned int num,sz;

    num=hp->ZZds.ZZeArr.num+1; sz=hp->ZZds.ZZeArr.sz;
    if(num>=sz)sizeChange(hp,num+1,0);
    hp->ZZds.ZZeArr.array[num]=(*e); (hp->ZZds.ZZeArr.num)++;
}
void** eArr_Array::pop(Department *hp){void** e=NULL; int num=hp->ZZds.ZZeArr.num;
    if(num>=0){e= &(hp->ZZds.ZZeArr.array[num]); (hp->ZZds.ZZeArr.num)--;}
    return e;
}
void eArr_Array::reset(Department *hp,int newSz,int incr){
    hp->ZZds.ZZeArr.num=newSz-1; hp->ZZds.ZZeArr.incr=incr;
}
...
// -----------------------------------------------------------------------
void eArr_Array::addOrd(cmpType cmpF,Department *hp,void* *op){
    int k,found;

    if(hp->ZZds.ZZeArr.num<0)k=0;
...
// elements.
// -----------------------------------------------------------------------
void eArr_Array::delOrd(cmpType cmpF,Department *hp,void* *obj){
    int k,found;

    k=binSearch(cmpF,hp,obj,&found);
    if(found) delOrd(cmpF,hp,k);
    else printf("warning: calling eArr::delOrd() but object not there\n");
}
...
// -----------------------------------------------------------------------
int eArr_Array::binSearch(cmpType cmpF,Department *hp,void* *op,int *found){
    int i1,i2,k,cmp,num; void** rr; void** arr;

    arr=hp->ZZds.ZZeArr.array;
    num=hp->ZZds.ZZeArr.num;
    if(!arr || num<0){
        printf("eArr error in binSearch(): array not formed or empty\n");
        return  -1;
    }
    i1=0; i2=num; k= -1;
...
// -----------------------------------------------------------------------
int eArr_Array::getBit(Department *hp,int i){
    int iChar,iBit,sz,k,newBytes; void** ap; unsigned char *p,b;

    sz=sizeof(void*) * hp->ZZds.ZZeArr.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ap=ind(hp,i/(8*sizeof(void*)));
        if(!ap){
            printf("eArr error: cannot realocate for %d bits\n",i);
            return -1;
        }

        // initialize the new part of the array to 0
        newBytes=sizeof(void*)*hp->ZZds.ZZeArr.sz;
        p=(unsigned char*)(hp->ZZds.ZZeArr.array);
        for(k=sz; k<newBytes; k++) p[k]=0;
    }
    else if(hp->ZZds.ZZeArr.num<0){ // first call, initialize all bits to 0
        p=(unsigned char*)(hp->ZZds.ZZeArr.array);
        for(k=0; k<sz; k++) p[k]=0;
    }

    k=i/(8*sizeof(void*)); // object index in which bit i is
    if(hp->ZZds.ZZeArr.num < k)hp->ZZds.ZZeArr.num=k;
...
// -----------------------------------------------------------------------
int eArr_Array::setBit(Department *hp,int i,int val){
    int iChar,iBit,sz,k,newBytes; void** ap; unsigned char *p,b;
...
// -----------------------------------------------------------------------
int eArr_Array::getOrd(cmpType cmpF,Department *hp,void* *op,int *found){

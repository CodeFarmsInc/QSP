#include <stdlib.h>
// --------------------------------------------------------
unsigned int oArr_Array::maxCapacity=0;
...
// --------------------------------------------------------------
void oArr_Array::extract(Header *hp, const int k){
    int j,num;

    num=hp->ZZds.ZZoArr.num;
    if(k>num)return; // nothing to do
    if(k==num){hp->ZZds.ZZoArr.num=num-1; return;}
    for(j=k+1; j<=num; j++)hp->ZZds.ZZoArr.array[j-1]=hp->ZZds.ZZoArr.array[j];
    hp->ZZds.ZZoArr.num=num-1;
}
...
// above this index. 
// --------------------------------------------------------------
void oArr_Array::insert(Header *hp,const int k,Employee *t){
    int j,kk,oldNum,newNum,sz;

    if(k<0)kk=0; else kk=k;
    oldNum=hp->ZZds.ZZoArr.num;
    sz=hp->ZZds.ZZoArr.sz;
    if(kk>oldNum) newNum=kk; else newNum=oldNum+1;
    if(newNum>=sz){
        (void)sizeChange(hp,newNum+1,0);
    }
    for(j=oldNum; j>=kk; j--) hp->ZZds.ZZoArr.array[j+1]=hp->ZZds.ZZoArr.array[j];
    hp->ZZds.ZZoArr.array[kk]=(*t);
    hp->ZZds.ZZoArr.num=newNum;
}
...
// --------------------------------------------------------------
int oArr_Array::sizeChange(Header *hp,const unsigned int newCap,const int exact){
    Employee* a; Employee* arr;
    int nCap,j,s,sn,cap,num,incr;

    nCap=newCap;
    arr=hp->ZZds.ZZoArr.array;
    cap=hp->ZZds.ZZoArr.sz;
    num=hp->ZZds.ZZoArr.num;
    incr=hp->ZZds.ZZoArr.incr;

    if(incr==0 && !exact){
    printf("error for oArr: attempt to change a fixed array into flexible one\n");
    return 1;
    }
    if(!arr){
        printf("error for oArr: cannot modify capacity, array not formed\n");
        return 1;
    }

    if(nCap==cap)return 0; // nothing to do
    if(maxCapacity>0 && nCap>maxCapacity){
        printf("error for oArr: requested capacity=%d, maxCapacity=%d\n",
                                                 nCap,maxCapacity);
        return 1;
    }
    ...
    a=new Employee[s];
    if(!a){
        printf("error for oArr: failed to allocate array capacity=%d\n",s);
        return 1;
    }
    ...
    delete[] arr;
    hp->ZZds.ZZoArr.array=a;
    hp->ZZds.ZZoArr.sz=s;
    if(num>=s)hp->ZZds.ZZoArr.num=s-1;
...
// --------------------------------------------------------------
Employee* oArr_Array::form(Header *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZoArr.array)free(hp);

    hp->ZZds.ZZoArr.array=new Employee[sz];
    if(!hp->ZZds.ZZoArr.array){
        printf("oArr error, cannot allocate array size=%d\n",sz);
        return NULL;
    }
    hp->ZZds.ZZoArr.sz=sz;
    hp->ZZds.ZZoArr.num= -1;
    hp->ZZds.ZZoArr.incr=incr;
    return hp->ZZds.ZZoArr.array;
}
...
// sorted.
// --------------------------------------------------------------
void oArr_Array::sort(cmpType cmp,Header *hp){
    if(hp->ZZds.ZZoArr.num<=0)return; // no need to sort 0 or 1 item
    qsort(hp->ZZds.ZZoArr.array,hp->ZZds.ZZoArr.num+1,sizeof(Employee),cmp);
}
void oArr_Array::sortSubset(cmpType cmp,Header *hp,int i1,int i2){
    if(i2>hp->ZZds.ZZoArr.num){
        printf("warning: upper range of sortSubset() automatically adjusted ");
        printf("from=%d to %d\n",i2,hp->ZZds.ZZoArr.num);
        i2=hp->ZZds.ZZoArr.num;
    }
    qsort(&(hp->ZZds.ZZoArr.array[i1]),i2-i1+1,sizeof(Employee),cmp);
}
...
// --------------------------------------------------------------
Employee* oArr_Array::ind(Header *hp,int i){
    if(!(hp->ZZds.ZZoArr.array)){
        printf("error for oArr: index operation attempted before formed\n");
        return NULL;
    }
    if(i>=hp->ZZds.ZZoArr.sz){
        if(sizeChange(hp,i+1,0))return NULL; // error message inside there
    }
    if(hp->ZZds.ZZoArr.num<i)hp->ZZds.ZZoArr.num=i;
    return &(hp->ZZds.ZZoArr.array[i]);
}

// --------------------------------------------------------------
// Fast way of getting ind[0]
// --------------------------------------------------------------
Employee* oArr_Array::head(Header *hp){
    return (Employee*)(hp->ZZds.ZZoArr.array);
}
...
// --------------------------------------------------------------
void oArr_Array::inHeap(cmpType cmpF,Header *hp,Employee *e,bck callback){
    int i; Employee* newE;

    i=hp->ZZds.ZZoArr.num + 1;
    newE=indHeap(hp,i,callback);
    if(!newE)return;
    (*newE)=(*e);
    if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[i]),i);
...
// --------------------------------------------------------------
void oArr_Array::upHeap(cmpType cmpF,Header *hp,int n,bck callback){
    ...
    for(i1=n, i2=(n-1)/2, a1=ind(hp,n); i1>0; i1=i2, i2=(i2-1)/2, a1=a2){
        ...
        aa=(*a1); (*a1)=(*a2); (*a2)=aa;
        if(callback){
            (*callback)(&(hp->ZZds.ZZoArr.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZoArr.array[i2]),i2);
        }
    }
}
...
// --------------------------------------------------------------
int oArr_Array::outHeap(cmpType cmpF,Header *hp,Employee* e,bck callback){
    int num;

    num=hp->ZZds.ZZoArr.num;
    if(num<0) return 0;
    (*e)=hp->ZZds.ZZoArr.array[0];
    if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[0]),-1);
    if(num>0){
       hp->ZZds.ZZoArr.array[0]=hp->ZZds.ZZoArr.array[num];
       if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[0]),0);
...
// --------------------------------------------------------------
void oArr_Array::downHeap(cmpType cmpF,Header *hp,int n,bck callback){
    ...
    for(k=n, ak=ind(hp,n); k<num; k=i1, ak=a1){
        ...
        if(callback){
            (*callback)(&(hp->ZZds.ZZoArr.array[i1]),i1);
            (*callback)(&(hp->ZZds.ZZoArr.array[k]),k);
        }
    }
}
...
// --------------------------------------------------------------
void oArr_Array::updHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num,n1,n2;

    num=hp->ZZds.ZZoArr.num;
    if(n<0 || n>num){
        printf("error updating heap=oArr, n=%d num=%d\n",n,num);
        return;
    }
...
// --------------------------------------------------------------
void oArr_Array::delHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num; Employee* arr;

    num=hp->ZZds.ZZoArr.num;
    arr=hp->ZZds.ZZoArr.array;
    if(n<0 || n>num){
        printf("error deleting from heap=oArr, n=%d num=%d\n",n,num);
        return;
    }
    (hp->ZZds.ZZoArr.num)--;
    if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[num]),-1);
    if(n==num) return;
    arr[n]=arr[num];
    if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[n]),n);
...
// --------------------------------------------------------------
Employee* oArr_Array::indHeap(Header *hp,int i,bck callback){
    int k,num,sz;
    Employee *elem;
    ...
    if(i>=sz){
        for(k=0; k<num; k++){
            if(callback) (*callback)(&(hp->ZZds.ZZoArr.array[k]),k);
        }
    }
    return elem;
}
...
int oArr_Array::formed(Header *hp){ if(hp->ZZds.ZZoArr.array)return 1; return 0;}

void oArr_Array::free(Header *hp){ 
    if(hp->ZZds.ZZoArr.array){
        delete[] hp->ZZds.ZZoArr.array;
        hp->ZZds.ZZoArr.array=NULL;
    }
    hp->ZZds.ZZoArr.num= -1;
    hp->ZZds.ZZoArr.sz=0;
}

unsigned int oArr_Array::capacity(Header *hp) {return hp->ZZds.ZZoArr.sz;} 

unsigned int oArr_Array::size(Header *hp) {return (unsigned int)(hp->ZZds.ZZoArr.num + 1);} 

int oArr_Array::increment(Header *hp) {return hp->ZZds.ZZoArr.incr;} 

Employee oArr_Array::get(Header *hp,const unsigned int k){  // a=array[k]
    int kk=k;
    if(kk>=hp->ZZds.ZZoArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZoArr.num)hp->ZZds.ZZoArr.num=kk;
    return hp->ZZds.ZZoArr.array[kk];
}

void oArr_Array::set(Header *hp,const unsigned int k,Employee a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZoArr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZoArr.num)hp->ZZds.ZZoArr.num=kk;
    hp->ZZds.ZZoArr.array[kk]=a;
}
void oArr_Array::remove(Header *hp,const unsigned int k){ // fast but order changed
    int kk=k;
    int num=hp->ZZds.ZZoArr.num;
    if(kk<num){hp->ZZds.ZZoArr.array[k]=hp->ZZds.ZZoArr.array[num]; (hp->ZZds.ZZoArr.num)--;}
}
int oArr_Array::reduce(Header *hp){             // reduce the array to its used size
    return sizeChange(hp,hp->ZZds.ZZoArr.num+1,1);
}
int oArr_Array::reduce(Header *hp,const unsigned int newCap){ // reduce size to newSz
    if(newCap>=hp->ZZds.ZZoArr.sz)return hp->ZZds.ZZoArr.sz;
    return sizeChange(hp,newCap,1);
} 
int oArr_Array::grow(Header *hp,const unsigned int newCap){
    return sizeChange(hp,newCap,0);
} 
void oArr_Array::push(Header *hp,Employee* e){
    unsigned int num,sz;

    num=hp->ZZds.ZZoArr.num+1; sz=hp->ZZds.ZZoArr.sz;
    if(num>=sz)sizeChange(hp,num+1,0);
    hp->ZZds.ZZoArr.array[num]=(*e); (hp->ZZds.ZZoArr.num)++;
}
Employee* oArr_Array::pop(Header *hp){Employee* e=NULL; int num=hp->ZZds.ZZoArr.num;
    if(num>=0){e= &(hp->ZZds.ZZoArr.array[num]); (hp->ZZds.ZZoArr.num)--;}
    return e;
}
void oArr_Array::reset(Header *hp,int newSz,int incr){
    hp->ZZds.ZZoArr.num=newSz-1; hp->ZZds.ZZoArr.incr=incr;
}
...
// -----------------------------------------------------------------------
void oArr_Array::addOrd(cmpType cmpF,Header *hp,Employee *op){
    int k,found;

    if(hp->ZZds.ZZoArr.num<0)k=0;
...
// elements.
// -----------------------------------------------------------------------
void oArr_Array::delOrd(cmpType cmpF,Header *hp,Employee *obj){
    int k,found;

    k=binSearch(cmpF,hp,obj,&found);
    if(found) delOrd(cmpF,hp,k);
    else printf("warning: calling oArr::delOrd() but object not there\n");
}
...
// -----------------------------------------------------------------------
int oArr_Array::binSearch(cmpType cmpF,Header *hp,Employee *op,int *found){
    int i1,i2,k,cmp,num; Employee* rr; Employee* arr;

    arr=hp->ZZds.ZZoArr.array;
    num=hp->ZZds.ZZoArr.num;
    if(!arr || num<0){
        printf("oArr error in binSearch(): array not formed or empty\n");
        return  -1;
    }
    i1=0; i2=num; k= -1;
...
// -----------------------------------------------------------------------
int oArr_Array::getBit(Header *hp,int i){
    int iChar,iBit,sz,k,newBytes; Employee* ap; unsigned char *p,b;

    sz=sizeof(Employee) * hp->ZZds.ZZoArr.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ap=ind(hp,i/(8*sizeof(Employee)));
        if(!ap){
            printf("oArr error: cannot realocate for %d bits\n",i);
            return -1;
        }

        // initialize the new part of the array to 0
        newBytes=sizeof(Employee)*hp->ZZds.ZZoArr.sz;
        p=(unsigned char*)(hp->ZZds.ZZoArr.array);
        for(k=sz; k<newBytes; k++) p[k]=0;
    }
    else if(hp->ZZds.ZZoArr.num<0){ // first call, initialize all bits to 0
        p=(unsigned char*)(hp->ZZds.ZZoArr.array);
        for(k=0; k<sz; k++) p[k]=0;
    }

    k=i/(8*sizeof(Employee)); // object index in which bit i is
    if(hp->ZZds.ZZoArr.num < k)hp->ZZds.ZZoArr.num=k;
...
// -----------------------------------------------------------------------
int oArr_Array::setBit(Header *hp,int i,int val){
    int iChar,iBit,sz,k,newBytes; Employee* ap; unsigned char *p,b;
...
// -----------------------------------------------------------------------
int oArr_Array::getOrd(cmpType cmpF,Header *hp,Employee *op,int *found){

...
// --------------------------------------------------------------
void arr_Array::extract(Header *hp, const int k){
    int j,num;

    num=hp->ZZds.ZZarr.num;
    if(k>num)return; // nothing to do
    if(k==num){hp->ZZds.ZZarr.num=num-1; return;}
...
// above this index. 
// --------------------------------------------------------------
void arr_Array::insert(Header *hp,const int k,Couple *t){
...
// --------------------------------------------------------------
int arr_Array::sizeChange(Header *hp,const unsigned int newCap,const int exact){
    Couple* a; Couple* arr;
...
// --------------------------------------------------------------
Couple* arr_Array::form(Header *hp,unsigned int const sz,int const incr){
    if(hp->ZZds.ZZarr.array)free(hp);

    hp->ZZds.ZZarr.array=new Couple[sz];
...
// sorted.
// --------------------------------------------------------------
void arr_Array::sort(cmpType cmp,Header *hp){
    if(hp->ZZds.ZZarr.num<=0)return; // no need to sort 0 or 1 item
    qsort(hp->ZZds.ZZarr.array,hp->ZZds.ZZarr.num+1,sizeof(Couple),cmp);
}
void arr_Array::sortSubset(cmpType cmp,Header *hp,int i1,int i2){
    if(i2>hp->ZZds.ZZarr.num){
        printf("warning: upper range of sortSubset() automatically adjusted ");
        printf("from=%d to %d\n",i2,hp->ZZds.ZZarr.num);
        i2=hp->ZZds.ZZarr.num;
    }
    qsort(&(hp->ZZds.ZZarr.array[i1]),i2-i1+1,sizeof(Couple),cmp);
}
...
// --------------------------------------------------------------
Couple* arr_Array::ind(Header *hp,int i){
    if(!(hp->ZZds.ZZarr.array)){
...
// --------------------------------------------------------------
Couple* arr_Array::head(Header *hp){
    return (Couple*)(hp->ZZds.ZZarr.array);
}
...
// --------------------------------------------------------------
void arr_Array::inHeap(cmpType cmpF,Header *hp,Couple *e,bck callback){
    int i; Couple* newE;
...
// --------------------------------------------------------------
void arr_Array::upHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int i1,i2; Couple* a1; Couple* a2; Couple aa;
...
// --------------------------------------------------------------
int arr_Array::outHeap(cmpType cmpF,Header *hp,Couple* e,bck callback){
    int num;
...
// --------------------------------------------------------------
void arr_Array::downHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int k,i1,i2,num; Couple* ak; Couple* a1; Couple* a2; Couple aa;
...
// --------------------------------------------------------------
void arr_Array::updHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num,n1,n2;
...
// --------------------------------------------------------------
void arr_Array::delHeap(cmpType cmpF,Header *hp,int n,bck callback){
    int num; Couple* arr;
...
// --------------------------------------------------------------
Couple* arr_Array::indHeap(Header *hp,int i,bck callback){
    int k,num,sz;
    Couple *elem;
...
int arr_Array::formed(Header *hp){ if(hp->ZZds.ZZarr.array)return 1; return 0;}

void arr_Array::free(Header *hp){ 
    if(hp->ZZds.ZZarr.array){
        delete[] hp->ZZds.ZZarr.array;
...
unsigned int arr_Array::capacity(Header *hp) {return hp->ZZds.ZZarr.sz;} 

unsigned int arr_Array::size(Header *hp) {return (unsigned int)(hp->ZZds.ZZarr.num + 1);} 

int arr_Array::increment(Header *hp) {return hp->ZZds.ZZarr.incr;} 

Couple arr_Array::get(Header *hp,const unsigned int k){  // a=array[k]
    int kk=k;
    if(kk>=hp->ZZds.ZZarr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZarr.num)hp->ZZds.ZZarr.num=kk;
    return hp->ZZds.ZZarr.array[kk];
}

void arr_Array::set(Header *hp,const unsigned int k,Couple a){ // array[k]=a
    int kk=k;
    if(kk>=hp->ZZds.ZZarr.sz)sizeChange(hp,kk+1,0);
    if(kk>hp->ZZds.ZZarr.num)hp->ZZds.ZZarr.num=kk;
    hp->ZZds.ZZarr.array[kk]=a;
}
void arr_Array::remove(Header *hp,const unsigned int k){ // fast but order changed
    int kk=k;
    int num=hp->ZZds.ZZarr.num;
    if(kk<num){hp->ZZds.ZZarr.array[k]=hp->ZZds.ZZarr.array[num]; (hp->ZZds.ZZarr.num)--;}
}
int arr_Array::reduce(Header *hp){             // reduce the array to its used size
    return sizeChange(hp,hp->ZZds.ZZarr.num+1,1);
}
int arr_Array::reduce(Header *hp,const unsigned int newCap){ // reduce size to newSz
    if(newCap>=hp->ZZds.ZZarr.sz)return hp->ZZds.ZZarr.sz;
    return sizeChange(hp,newCap,1);
} 
int arr_Array::grow(Header *hp,const unsigned int newCap){
    return sizeChange(hp,newCap,0);
} 
void arr_Array::push(Header *hp,Couple* e){
    unsigned int num,sz;
...
Couple* arr_Array::pop(Header *hp){Couple* e=NULL; int num=hp->ZZds.ZZarr.num;
    if(num>=0){e= &(hp->ZZds.ZZarr.array[num]); (hp->ZZds.ZZarr.num)--;}
    return e;
}
void arr_Array::reset(Header *hp,int newSz,int incr){
...
// -----------------------------------------------------------------------
void arr_Array::addOrd(cmpType cmpF,Header *hp,Couple *op){
    int k,found;
...
// elements.
// -----------------------------------------------------------------------
void arr_Array::delOrd(cmpType cmpF,Header *hp,Couple *obj){
    int k,found;
...
// -----------------------------------------------------------------------
int arr_Array::binSearch(cmpType cmpF,Header *hp,Couple *op,int *found){
    int i1,i2,k,cmp,num; Couple* rr; Couple* arr;

    arr=hp->ZZds.ZZarr.array;
    num=hp->ZZds.ZZarr.num;
    if(!arr || num<0){
        printf("arr error in binSearch(): array not formed or empty\n");
        return  -1;
    }
    i1=0; i2=num; k= -1;
...
// -----------------------------------------------------------------------
int arr_Array::getBit(Header *hp,int i){
    int iChar,iBit,sz,k,newBytes; Couple* ap; unsigned char *p,b;

    sz=sizeof(Couple) * hp->ZZds.ZZarr.sz; // sz is size in bytes
    if(i>=sz*8){ // sz*8 is the size in bits
        ap=ind(hp,i/(8*sizeof(Couple)));
        if(!ap){
        ...
        newBytes=sizeof(Couple)*hp->ZZds.ZZarr.sz;
    ...
    k=i/(8*sizeof(Couple)); // object index in which bit i is
...
// -----------------------------------------------------------------------
int arr_Array::setBit(Header *hp,int i,int val){
    int iChar,iBit,sz,k,newBytes; Couple* ap; unsigned char *p,b;
...
// -----------------------------------------------------------------------
int arr_Array::getOrd(cmpType cmpF,Header *hp,Couple *op,int *found){

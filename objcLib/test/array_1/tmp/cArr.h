...
// EQUIVALENT OF:
// template <class Holder,class Element> class cArr_Array
// --------------------------------------------------------------------
// Implementation note:
...
#ifndef ZZ_cArr_ARRAY_INCLUDED
#define ZZ_cArr_ARRAY_INCLUDED

// ----------------------------------------------------------
// description of the cooperating classes
// ----------------------------------------------------------
class cArr_ArrayHolder {
public:
    unsigned int sz; // present size
    int num,incr; // higest valid index,increment
    int* array;
    cArr_ArrayHolder(){ sz=0; num= -1; incr=0; array=NULL;}
};
...
protected:
    ...
    static int* indHeap(Header *hp,int i,bck callback); // like ind() but callback
    ...
    static int binSearch(cmpType cmpF,Header *hp,int *op,int *found);
public:
    static void setMaxCap(unsigned int maxCap){maxCapacity=maxCap;}
    static int* form(Header *hp,unsigned int const sz,int const incr);
    ...
    static int get(Header *hp,const unsigned int k);  // a=array[k]
    static void set(Header *hp,const unsigned int k,int a); // array[k]=a
    ...
    static void insert(Header *hp,const int k,int *t); 
    ...
    static int* ind(Header *hp,int i);
    static void addOrd(cmpType cmpF,Header *hp,int *op);

    // array as a stack 
    // ------------------------------------------------------------
    static void push(Header *hp,int* e);
    static int* pop(Header *hp);

    // array as a partially sorted, binary heap
    // -----------------------------------------
    static void inHeap(cmpType cmpF,Header *hp,int *e,bck callback);
    static int outHeap(cmpType cmpF,Header *hp,int *e,bck callback);
    ...
    static int* head(Header *hp); //fast way of getting array[0]
    static void reset(Header *hp,int newSz,int incr);
    static void ins(Header *hp,int k,int* t){insert(hp,k,t);}
    static void delOrd(cmpType cmpF,Header *hp,int k){ extract(hp,k); }
    static void delOrd(cmpType cmpF,Header *hp,int *obj);
    static int getBit(Header *hp,int i);
    static int setBit(Header *hp,int i,int val);
    static int getOrd(cmpType cmpF,Header *hp,int *op,int *found);
};

#endif // ZZ_cArr_ARRAY_INCLUDED

...
// EQUIVALENT OF:
// template <class Holder,class Element> class myArray_Array
// --------------------------------------------------------------------
// Implementation note:
...
#ifndef ZZ_myArray_ARRAY_INCLUDED
#define ZZ_myArray_ARRAY_INCLUDED

// ----------------------------------------------------------
// description of the cooperating classes
// ----------------------------------------------------------
class myArray_ArrayHolder {
public:
    unsigned int sz; // present size
    int num,incr; // higest valid index,increment
    Entry* array;
    myArray_ArrayHolder(){ sz=0; num= -1; incr=0; array=NULL;}
};
...
protected:
    ...
    static Entry* indHeap(Header *hp,int i,bck callback); // like ind() but callback
    ...
    static int binSearch(cmpType cmpF,Header *hp,Entry *op,int *found);
public:
    static void setMaxCap(unsigned int maxCap){maxCapacity=maxCap;}
    static Entry* form(Header *hp,unsigned int const sz,int const incr);
    ...
    static Entry get(Header *hp,const unsigned int k);  // a=array[k]
    static void set(Header *hp,const unsigned int k,Entry a); // array[k]=a
    ...
    static void insert(Header *hp,const int k,Entry *t); 
    ...
    static Entry* ind(Header *hp,int i);
    static void addOrd(cmpType cmpF,Header *hp,Entry *op);

    // array as a stack 
    // ------------------------------------------------------------
    static void push(Header *hp,Entry* e);
    static Entry* pop(Header *hp);

    // array as a partially sorted, binary heap
    // -----------------------------------------
    static void inHeap(cmpType cmpF,Header *hp,Entry *e,bck callback);
    static int outHeap(cmpType cmpF,Header *hp,Entry *e,bck callback);
    ...
    static Entry* head(Header *hp); //fast way of getting array[0]
    static void reset(Header *hp,int newSz,int incr);
    static void ins(Header *hp,int k,Entry* t){insert(hp,k,t);}
    static void delOrd(cmpType cmpF,Header *hp,int k){ extract(hp,k); }
    static void delOrd(cmpType cmpF,Header *hp,Entry *obj);
    static int getBit(Header *hp,int i);
    static int setBit(Header *hp,int i,int val);
    static int getOrd(cmpType cmpF,Header *hp,Entry *op,int *found);
};

#endif // ZZ_myArray_ARRAY_INCLUDED

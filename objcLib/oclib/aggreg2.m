...
{
    if(c->$0.parent){
        printf("$$.addHead() error: Child=%llu already in $$_Aggregate2\n",c);
        return;
    }
    c->$0.parent=p;
...
{
    if(c->$0.parent){
        printf("$$.addTail() error: Child=%llu already in $$_Aggregate2\n",c);
        return;
    }
    c->$0.parent=p;
...
+ (void) append: ($2*) c1 elem: ($2*) c2
{
    $1* p=c1->$0.parent;
    if(!p){
        printf("$$.append() error: c1=%llu not in $$_Aggregate2\n",(U)c1);
        return;
    }
    if(c2->$0.parent){
        printf("$$.addTail() error: c2=%llu already in $$_Aggregate2\n",(U)c2);
        return;
    }
    c2->$0.parent=p;
...
+ (void) insert: ($2*) c1 before: ($2*) c2
{
    $1* p=c2->$0.parent;
    if(!p){
        printf("$$.append() error: c2=%llu not in $$_Aggregate2\n",(U)c2);
        return;
    }
    if(c1->$0.parent){
        printf("$$.addTail() error: c1=%llu already in $$_Aggregate2\n",(U)c1);
        return;
    }
    c1->$0.parent=p;
...
+ (void) remove: ($2*) c{
    ...
    else printf("WARNING: $$.remove() with c=%llu already disconnected\n",(U)c);
}

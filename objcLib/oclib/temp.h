...
+ ($1*) const next: ($1*) p elem: ($1*) c; // returns NULL when p is the tail
+ ($1*) const prev: ($1*) p elem: ($1*) c; // returns NULL when p is the head
+ ($1*) const nextRing: ($1*) c; // next in the ring
+ ($1*) const prevRing: ($1*) c; // previous in the ring
+ ($1*) sort: (ZZsortFun) cmpFun tail: ($1*) tail; // returns the new tail
+ (void merge: ($1*) s second: ($1*) t;
@end

@interface $$_Ring2Iterator
{
$1 *tail; // NULL when loop finished
$1 *nxt;  // NULL when starting a new loop
